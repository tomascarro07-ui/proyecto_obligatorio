...
function mostarVentas() {
	...
	for(let i = 0; i < ventasRealizadas.length; i++) {
		...
		ventas += `<div class="card"> <h3>Cliente: ${venta.persona}</h3> <p>Fecha: ${venta.fecha}</p> <p>Cantidad de productos: ${venta.cantidadProductos}</p> Detallado: <br> ${venta.nombreProductos} <p>Metodo de pago: ${venta.metodoPago} <p> Metodo de envio: ${venta.metodoEntrega}<p>Subtotal: $${venta.subtotal}</p> <p>IVA: $${venta.iva}</p> <h3>Total: $${venta.sumaTotal.toFixed(2)}</h3> </div> `
